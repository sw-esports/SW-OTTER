    ...
    list.innerHTML = `
        ...
        <div class="controls">
            ...
            <button class="del-btn" contenteditable="false">
                <img src="/src/delete-bin-6-line (1).svg" alt="Delete">
            </button>
        </div>`;
...
function saveLists() {
    const leftLists = Array.from(leftDiv.querySelectorAll(".list")).map(list => list.querySelector("p").innerText);
    const rightLists = Array.from(rightDiv.querySelectorAll(".list")).map(list => list.querySelector("p").innerText);
